...
export default async function PddSection({ params }: { params: { section: string } }) {
  ...
  return <main className="mx-auto max-w-5xl space-y-8 p-4 md:p-8">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-gray-500">PDD / {params.section.toUpperCase()}</p><h1 className="text-2xl font-medium text-black">Kelola {section.name}</h1></div><LogoutButton /></div>
